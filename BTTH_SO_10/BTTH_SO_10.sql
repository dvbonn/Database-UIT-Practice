...
--CAU-3--------------------
--3.1----------------------
SELECT DG.MaDG, DG.HoTen FROM DOCGIA DG
JOIN PHIEUTHUE PT ON DG.MaDG = PT.MaDG
JOIN CHITIET_PT CT ON PT.MaPT = CT.MaPT
JOIN SACHD2 S ON CT.MaSach = S.MaSach
...
WHERE S.MaSach IN (
    ...
    FROM SACHD2 S2
    JOIN CHITIET_PT CT2 ON S2.MaSach = CT2.MaSach
    WHERE S2.TheLoai = S.TheLoai
    GROUP BY S2.MaSach
    ORDER BY COUNT(CT2.MaPT) DESC
)
...
CREATE TABLE CHITIET_PM (
	MaPT CHAR(5) REFERENCES PHIEUTHUE_BD,
	MaBD CHAR(5) REFERENCES BANG_DIA,
...
BEGIN 
	...
	BEGIN 
		RAISERROR (N'Chỉ khách hàng VIP mới được thuê trên 5 băng đĩa.', 16, 1)
		ROLLBACK TRANSACTION
	END
END
...
JOIN PHIEUTHUE_BD PT ON KH.MaKH = PT.MaKH
JOIN CHITIET_PM CT ON PT.MaPT = CT.MaPT
JOIN BANG_DIA BD ON CT.MaBD = BD.MaBD
WHERE BD.TheLoai = N'phim tình cảm'
	AND PT.Soluongthue > 3
GO

--3.2---
SELECT TOP 1 WITH TIES KH.MaKH, KH.HoTen FROM KHACHHANG KH
JOIN PHIEUTHUE_BD PT ON KH.MaKH = PT.MaKH
JOIN CHITIET_PM CT ON PT.MaPT = CT.MaPT
JOIN BANG_DIA BD ON CT.MaBD = BD.MaBD
WHERE KH.LoaiKH = 'VIP'
GROUP BY KH.MaKH, KH.HoTen
ORDER BY COUNT(CT.MaPT) DESC
GO

--3.3---
SELECT BD.TheLoai, KH.HoTen FROM KHACHHANG KH
JOIN PHIEUTHUE_BD PT ON KH.MaKH = PT.MaKH
JOIN CHITIET_PM CT ON PT.MaPT = CT.MaPT
JOIN BANG_DIA BD ON CT.MaBD = BD.MaBD
WHERE KH.MaKH IN(
		SELECT TOP 1 WITH TIES KH2.MaKH FROM KHACHHANG KH2
		JOIN PHIEUTHUE_BD PT2 ON KH2.MaKH = PT2.MaKH
		JOIN CHITIET_PM CT2 ON PT2.MaPT = CT2.MaPT
		JOIN BANG_DIA BD2 ON CT2.MaBD = BD2.MaBD
